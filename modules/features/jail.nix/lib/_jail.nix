...
let
  jailFn = inputs.jail-nix.lib.extend {
    pkgs = pkgs;
    additionalCombinators = c: with c; let
      gui-base = compose [
        gui gpu notifications
        (dbus {
          talk = [
  ...
  mkSandboxed = pkg: name: permissions:
    let jailed = jailFn name pkg permissions;
    in pkgs.runCommand ("${name}-sandboxed") { } ''
      cp -rs "${pkg}" "$out"
      if [ -L "$out/bin" ]; then
